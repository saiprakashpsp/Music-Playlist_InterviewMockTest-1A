...
class App extends Component {
  ...
  updateList = id => {
    ...
    const filterList = songsList.filter(eachItem => eachItem.id !== id)
    // console.log(filterList)
    this.setState({songsList: filterList})
  }
  ...
  render() {
    ...
    const onSearchFilteredList = songsList.filter(eachItem =>
      ...
      <div className="music-app-container">
        ...
        <div className="playlist-container">
          <div className="search-bar-container">
            <h1 className="heading-song">Songs Playlist</h1>
            <input
              type="search"
              value={searchInput}
              placeholder="Search"
          ...
          {onSearchFilteredList.length > 0 ? (
            <ul className="song-list-container">
              {onSearchFilteredList.map(eachItem => (
                <MusicPlaylist
                  key={eachItem.id}
